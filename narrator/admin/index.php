...
if(isset($_SESSION['logged_in'])){
	...
	<html>
		...
		<body>
			<div class="container">
			<h2><a href="/narrator/index.php" id="logo">Narrator</a></h2><br/>
				<ol>
	...
	if(isset($_POST['username'])&&isset($_POST['password'])){
		...
		if(!empty($username)&&!empty($password)){
			$query="SELECT `id` FROM `narrator`.`users` WHERE `username`= '".mysql_escape_string($username)."' AND `password`= '".mysql_escape_string($password)."'";
